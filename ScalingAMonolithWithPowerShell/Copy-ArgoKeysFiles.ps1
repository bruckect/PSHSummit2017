function Copy-ArgoKeysUpdates {
    ...
    $DPs | Invoke-Parallel -Throttle 60 -RunspaceTimeout 600 -ScriptBlock {
        Write-Verbose -Message "Starting RoboCopy to $_\$Using:argoShare"
        robocopy "\\$Using:argoServer\$Using:argoShare" "\\$_\$Using:argoShare" /MIR /R:0 /NFL /NDL
    }
}
...
$dps = Execute-SqlQuery -sqlServer sqlServer -databaseName FFB_SCCM_Utility -sqlQuery 'EXEC uspGetDistributionPoints' | Select-Object -ExpandProperty DistributionPoint

Copy-ArgoKeysUpdates -DPs $dps -Environment Production
Copy-ArgoKeysUpdates -DPs $dps -Environment Training
Copy-ArgoKeysUpdates -DPs $dps -Environment Test